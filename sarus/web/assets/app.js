...
async function initOverview(){
  ...
  byId('runtime-summary').innerHTML=`${badge(s.name+' '+s.version,'info')} ${badge(s.models?.online?'Ollama online':'Ollama offline',s.models?.online?'ok':'bad')} ${badge((s.providers?.mode||'local_only').replaceAll('_',' '),'info')} ${badge(s.host_broker?'Host broker ready':'Host broker unavailable',s.host_broker?'ok':'warn')} ${badge(s.receipt_chain?.ok?'Receipt chain verified':'Receipt chain issue',s.receipt_chain?.ok?'ok':'bad')}`;
...
async function initAgents(){if(byId('manager-run'))byId('manager-run').onclick=runAgentManager;byId('cap-search-btn').onclick=loadCapabilities;byId('cap-run-btn').onclick=runCapability;const s=await API.get('/api/status');byId('cap-source').innerHTML='<option value="">All sources</option>'+(s.adapters||[]).map(a=>`<option value="${esc(a.name)}">${esc(a.details?.label||a.name)}</option>`).join('');byId('agent-sources').innerHTML=(s.adapters||[]).map(a=>`<div class="source-card"><div class="split between"><strong>${esc(a.details?.label||a.name)}</strong>${badge(a.connected?'Connected':'Missing',a.connected?'ok':'bad')}</div><div class="data-meta">${esc(a.details?.role||'')}</div></div>`).join('');await loadCapabilities();}
async function runAgentManager(){const text=byId('manager-task').value.trim();if(!text)return toast('Enter a managed task','bad');const type=byId('manager-type').value||'auto',project=byId('manager-project').value.trim(),btn=byId('manager-run');setBusy(btn,true,'Running agents');try{const r=await API.post('/api/agent-manager/run',{text,task_type:type,project_path:project,provider:'ollama'});jsonBox('manager-output',r);toast(r.status==='completed'?'Agent task completed':'Agent task failed',r.status==='completed'?'ok':'bad');}catch(e){jsonBox('manager-output','Error: '+e.message);toast(e.message,'bad');}finally{setBusy(btn,false);}}
...
async function devAction(run){const text=byId('dev-input').value.trim(),project=byId('dev-project').value.trim();if(!text)return toast('Describe the development task','bad');if(!project)return toast('Enter an approved project path','bad');const btn=byId(run?'dev-run':'dev-plan');setBusy(btn,true,run?'Editing + verifying':'Planning');try{const r=run?await API.post('/api/development/run',{text,project_path:project,provider:'ollama'}):await API.post('/api/plan',{text:'Development task: '+text});jsonBox('dev-output',r);await loadDevTasks();toast(run?(r.status==='completed'?'Development run verified':'Development run did not pass verification'):'Plan created',r.status==='completed'||!run?'ok':'bad');}catch(e){jsonBox('dev-output','Error: '+e.message);}finally{setBusy(btn,false);}}
async function loadDevTasks(){const rows=await API.get('/api/development?limit=20');byId('dev-history').innerHTML=(rows||[]).map(x=>`<div class="data-row"><div class="data-main"><div class="data-title">${esc(short(x.request,130))}</div><div class="data-meta">${esc(x.project||'')} · ${fmtDate(x.ts)} · files: ${esc((x.changed_files||[]).join(', ')||'none')}</div></div>${badge(x.status,statusTone(x.status))}</div>`).join('')||renderEmpty('No development runs yet.');}
