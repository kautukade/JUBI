...
class WindowsBroker:
    ...
    def available(self):
        # The historical class name is retained for compatibility. Workspace
        # and bounded host-observation operations are cross-platform.
        return True
    ...
    def execute_typed(self, action_id: str, parameters: dict | None = None, resolved: dict | None = None):
        ...
        if action_id in {'service.query', 'service.start', 'service.stop'}:
            ...
            if not unit or not unit.endswith('.service') or any(ch in unit for ch in '"&|<>/\\\r\n'):
                raise ValueError('invalid allowlisted Linux service mapping')
            systemctl = shutil.which('systemctl')
            if not systemctl:
                return {'ok': False, 'error': 'systemctl is not available'}
            verb = action_id.split('.', 1)[1]
            argv = [systemctl, 'status' if verb == 'query' else verb, unit, '--no-pager']
            return self._run(argv, 30)
        ...
        if action_id == 'process.stop':
            ...
            signal = '-KILL' if parameters.get('force') else '-TERM'
            return self._run(['pkill', signal, '-x', name], 15)
