...
class Orchestrator:
    ...
    def plan(self,text):
        ...
        if any(x in t for x in ['code','bug','website','project','develop','fix','build']):
            add('developer-method','superpowers','create a TDD/debugging implementation plan and verification criteria',1); add('implementation-review','ecc','review the proposed implementation against source rules and safety',1); add('local-developer','hermes','execute the coding/project work with Jubi bounded VPS workspace tools, then verify and independently review it',2); add('code-review','ecc','review the resulting implementation and verification evidence',1)
